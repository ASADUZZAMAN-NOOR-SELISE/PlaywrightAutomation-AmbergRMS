...
export class JobPage {
  constructor(page) {
    this.page = page;
    this.addJobBtn = page.locator(".project-tree-job").getByText("Add Job");
    this.jobDrawer = page.getByTestId("add-job-section-drawer-test-id");
    this.job = page.getByTestId('job-tree-testid');
    ...
    //speed locator
    this.speed = page.getByTestId('job-tree-testid-child-0');
  ...
  async clickAddJob() {
    await expect(this.addJobBtn).toBeVisible();
  ...
  async verifyJobCreated(name, comment, date) {
    await expect(this.page.getByTestId('job-tree-testid')).toBeVisible();
    await this.page.getByTestId('job-tree-testid').getByText(name).click();
  ...
  async jobClick(name){
    await this.job.getByText(name).click();
  }
