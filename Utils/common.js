...
export class Common {
  constructor(page) {
    ...
    this.stationingStartInput = page.locator(
      "input[name='MetaData.Start.Stationing']",
    );
    this.stationingEndInput = page.locator(
      "input[name='MetaData.End.Stationing']",
    );
    this.commentInput = page.getByRole("textbox", { name: "Comment" });
    this.lineSectionNameInput = page.getByRole("textbox", {
      name: "Name of Line Section",
    });
    ...
    this.customerStreetInput = page.locator(
      'input[name="CustomerInfo.Street"]',
    );
    this.customerTownInput = page.locator('input[name="CustomerInfo.Town"]');
    this.customerPostalCodeInput = page.locator(
      'input[name="CustomerInfo.PostalCode"]',
    );
    this.customerRegionInput = page.locator(
      'input[name="CustomerInfo.Region"]',
    );
    this.customerCountryDropdown = page.locator(
      '[id="mui-component-select-CustomerInfo.Country"]',
    );
    this.customerPhoneInput = page.locator(
      'input[name="CustomerInfo.PhoneNumber"]',
    );
    this.customerEmailInput = page.locator('input[name="CustomerInfo.Email"]');
    // Service Provider
    this.serviceProviderNameInput = page.locator(
      'input[name="ServiceProviderInfo.Name"]',
    );
    this.serviceProviderStreetInput = page.locator(
      'input[name="ServiceProviderInfo.Street"]',
    );
    this.serviceProviderTownInput = page.locator(
      'input[name="ServiceProviderInfo.Town"]',
    );
    this.serviceProviderPostalCodeInput = page.locator(
      'input[name="ServiceProviderInfo.PostalCode"]',
    );
    this.serviceProviderRegionInput = page.locator(
      'input[name="ServiceProviderInfo.Region"]',
    );
    this.serviceCountryDropdown = page.getByLabel("", { exact: true });
    this.serviceProviderPhoneInput = page.locator(
      'input[name="ServiceProviderInfo.PhoneNumber"]',
    );
    this.serviceProviderEmailInput = page.locator(
      'input[name="ServiceProviderInfo.Email"]',
    );
    //country menu
    this.serviceProviderCountryBackdrop = page.locator(
      '[id="menu-ServiceProviderInfo.Country"] > .MuiBackdrop-root',
    );
    this.submitButton = page.locator("button[type='submit']");
    this.searchByProjectNameInput = page.getByRole("textbox", {
  ...
  async setProjectName(name) {
    ...
    await this.nameInput.pressSequentially(" " + name, { delay: 400 });
  }

  async generalInformation(data) {
    await this.nameInput.click();
    await this.page.waitForTimeout(1500);
    await this.nameInput.pressSequentially(" " + data.name, { delay: 200 });
